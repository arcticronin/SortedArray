...
void test2()
{
  ...
  // Test remove
  assert(0 == arr3.remove(Person("Tom", 40)));
  assert(arr3.size() == 1);
  assert(-1 == arr3.remove(Person("Luca", 29)));
  ...
  SortedArray<Person, AgeOrderPolicy, NameEqualPolicy> f = arr.filter([](const Person &p)                                                          { return p.age > 25; });
...
void test1()
{
  ...
  // Test filter
  SortedArray<int, std::less<int>, std::equal_to<int>> filtered = arr.filter([](int num)
                                                                             { return num % 2 == 0; });
...
void test0()
{
  ...
  void test3(){
  SortedArray<Person , AgeOrderPolicy, NameEqualPolicy> p2;
  p2.insert(Person("gino", 1));
  p2.insert(Person("gino2", 2));
  p2.insert(Person("gino3", 3));
  p2.insert(Person("gino4", 4));
  p2.insert(Person("gino5", 5));
  p2.insert(Person("gino6", 6));
  p2.insert(Person("gino7", 7));

  SortedArray<int, AscendingOrd, Equalz> p;
  p.insert(1);
  p.insert(2);
  p.insert(3);
  p.insert(4);
